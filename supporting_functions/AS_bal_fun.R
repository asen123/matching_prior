...
AS_func<-function(m,A,Di,y,X,i){
  ...
  l <- (det(V)^(-1/2))*(det(t(X)%*% VV1 %*%X)^(-1/2))*
    exp(-1/2*t(y)%*%P%*%y)
  # adjustment term
  #q_i <- t(X[i,])%*%ginv(t(X) %*% X) %*% X[i,]
  h <- (A^((1+z^2)/4))*((A+D)^((7-z^2)/4) #+ (m*q_i)/2
                        )
  return(l*h)
}
#==============================================================================
...
EBLUP_func_AS <- function(m,Di,X,yy){
  ...
  int_summ <- data.frame(A_hat_REML = rep(A.est[1],m),
                         ...
                         EBLUP_REML = Eblup[,1],
                         EBLUP_AS = Eblup[,2],
                         REML_lower = Eblup[,1] - z*sqrt(g1(m,A.est[1],Di)+g2(m,A.est[1],Di,X)),
                         REML_upper = Eblup[,1] + z*sqrt(g1(m,A.est[1],Di)+g2(m,A.est[1],Di,X)),
